...
public class Device implements org.cybergarage.http.HTTPRequestListener,
	...
	public boolean loadDescription(File file)
		...
		try {
			...
			System.out.println("rootNode xmlns="
					+ rootNode.getAttributeValue("xmlns"));
			deviceNode = rootNode.getNode(Device.ELEM_NAME);
			if (deviceNode == null)
				throw new InvalidDescriptionException(
	...
	private synchronized byte[] getDescriptionData(String host) {
		...
		desc += "\n";
		desc += rootNode.toString(); // 输出节点
		// desc += "<root xmlns=\"urn:schemas-upnp-org:device-1-0\">";
	...
	public boolean start() {
        ...
        System.out.println("start: Device.java start() stack trace =================");
        for (StackTraceElement ste : Thread.currentThread().getStackTrace()) {
            System.out.println(ste);
        }
        System.out.println("end:   Device.java start() stack trace =================\n\n");
